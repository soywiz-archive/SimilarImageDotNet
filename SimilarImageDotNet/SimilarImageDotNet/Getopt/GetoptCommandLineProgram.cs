	...
	abstract public class GetoptCommandLineProgram
	{
		...
		virtual protected void ShowHelp()
		{
			...
			foreach (var CommandEntry in CommandEntries)
			{
				...
				Console.Write("{0}", String.Join(", ", CommandEntry.Aliases.Take(1)));
				if (CommandEntry.MethodInfo != null)
				{
					if (CommandEntry.MethodInfo.GetParameters().Length > 0)
					{
						Console.Write(" <{0}>", String.Join(", ", CommandEntry.MethodInfo.GetParameters().Select(Item =>
						{
				...
				{
					Console.Write(" [{0}]", String.Join(", ", CommandEntry.Values));
				}
		...
		private void BindField(CommandEntry CommandEntry)
		{
			...
			Getopt.AddRule(CommandEntry.Aliases, () =>
			{
				...
				else if (Field.FieldType == typeof(int))
				{
					...
					if (!CommandEntry.Values.Contains(Value)) throw (new Exception(String.Format("Expecting one of [{0}] but found {1}", String.Join(", ", CommandEntry.Values), Value)));
		...
		static private T GetSingleAttribute<T>(MemberInfo MemberInfo)
		{
